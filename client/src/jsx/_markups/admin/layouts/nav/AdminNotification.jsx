...
import "../../../guest/app-assets/css/style.scss";
...
export default function AdminNotification({ hideNotification = false }) {
  const {
    session: { user },
    socket,
  ...
  const [adminNotification, setNotification] = useState([]);
  const [readCount, setReadCount] = useState(0);
  const [offset, setOffset] = useState(0);
  const [fetchMore, setFetchMore] = useState(true);
  const onSocketConnection = () => {
    socket.emit("notification::join");
    // notification::adminkyc
    socket.on("notification::adminnew", (res) => {
      console.log(res);
    ...
    socket.on("notification::adminnewwithdraw", (res) => {
      console.log(res);
      setNotification((prevState) => {
        return [res, ...prevState];
      });
  ...
  const openNotification = (id, link) => {
    socket.emit("notification::read::admin", { id: id }, (res) => {
  ...
  function onNotificationClear(id, link) {
    socket.emit("notification::delete::admin", { id: id }, (res) => {
      console.log(res);
      // markNotificationAsRead(res);
      let obj = adminNotification?.find((o) => o.id == res.id);
      let ind = adminNotification.indexOf(obj);
      const tempArray = [...adminNotification];
      tempArray.splice(ind, 1);
      setNotification(tempArray);
      // setReadCount(tempArray?.length);
    });
  }
  useEffect(() => {
    if (socket.connected) {
  ...
  //         res = res;
  //         // console.log("notification::adminkyc  ", res)
  //         // setNotification((prev) => {  
  //         //   console.log(notification)
  //         //   console.log(res.rows)
  //         //   console.log([...res.rows, ...prev])
  //         //   return [...res.rows, ...prev];
  //         // });

  //         res.rows.forEach(element => {
  //           adminNotification.push(element);
  //         });

  //         // setNotification([...res.rows]);
  //         // console.log(notification)
  //         // console.log(res.rows)
  //         // let a = [{
  //         //   "a": "a",
  ...
  //         //   "b": "b"
  //         // }]
  //         // let newArray = [...notification, ...res.rows];
  //         // // let newArray = [...a,...b];
  //         // console.log(newArray);
  //         // if(notification.length > 0){
  //         //   console.log("setNotification ")
  //         //   // setNotification(newArray)
  //         //   setNotification([...notification, ...res.rows])
  //         // }else{
  //         //   console.log("setNotification else ")
  //         //   setNotification(["a","b"])
  //         // }
                  ...
                  .map((item, idx) => (
                    ...
                    >
                      <div style={{ display: "flex", "justify-content": "space-between" }}>
                        ...
                        <span className="btn btn-sm" onClick={(e) => { onNotificationClear(item?.id, item?.link) }}><i className="fa fa-close "></i></span>
                      </div>
